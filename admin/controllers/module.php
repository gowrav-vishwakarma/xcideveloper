...
//        $admin=($this->input->get('xcidevelop_side')=="")? "" : $this->input->get('xcidevelop_side')."\\";
        ...
        $this->session->set_userdata("xCI_home_dir",JPATH_SITE.DS."modules\mod_".$prj->component);
...
//        $this->zip->download('com_'.$prj->component.'.zip');
    ...
    function saveNewModule(){
        ...
        $prjDetails=array(
                ...
                'version'=>$this->input->get('version'),
                'description'=>$this->input->get('description'),
                'atchwith'=>$this->input->get('atchWith')
                );
        $prjConfig=array(
        ...
        if(!$this->mod_creator->createNewModule($safefoldername,$prjDetails,$prjConfig))
                xRedirect("index.php?option=com_xcideveloper","Module Already Exists.. Either Uninstall it or remove directory","error");


        xRedirect("index.php?option=com_xcideveloper","Module Created and Registered, Start developing or managing the module");
    }
